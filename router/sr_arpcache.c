...
void sr_arpcache_handle_arpreq(struct sr_instance *sr, struct sr_arpreq *req) {
	...
	if (difftime(curtime, req->sent) > 1.0) {
		...
		/**************** fill in code here *****************/
            ...
            fprintf(stderr, " (ARP Request Header...)\n\n");
            ...
            memset(a_hdr->ar_tha, 255, ETHER_ADDR_LEN);
            a_hdr->ar_sip = ifc->ip;
            a_hdr->ar_tip = req->ip;

            fprintf(stderr, " (ARP Ethernet Header...)\n\n");
            e_hdr = (sr_ethernet_hdr_t *)malloc(sizeof(sr_ethernet_hdr_t));
            memcpy(e_hdr->ether_shost, ifc->addr, ETHER_ADDR_LEN);
            memset(e_hdr->ether_dhost, 255, ETHER_ADDR_LEN);
            e_hdr->ether_type = htons(ethertype_arp);

            len = sizeof(sr_ethernet_hdr_t) + sizeof(sr_arp_hdr_t);
            buf = malloc(len);
            memcpy(buf, e_hdr, sizeof(sr_ethernet_hdr_t));
            memcpy(buf + sizeof(sr_ethernet_hdr_t), a_hdr, sizeof(sr_arp_hdr_t));
            /* send */
